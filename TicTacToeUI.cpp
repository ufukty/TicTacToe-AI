...
namespace TicTacToe {
	...
	void UI::header(Game_Data &game)
	{
		...
		std::string right = "KEYS: W, A, S, D, SPACE, H";
	...
	void UI::render_menu_screen(Game_Data &game)
	{
		...
		if (game.status == GAME_MENU_SCREEN_1) {
			...
			repeat_print(6, '\n');
			std::string command = "Select board size";
			centered(command);
			...
			repeat_print(6, '\n');
			std::string command = "Pick your token (X starts first)";
			centered(command);
